"""Routine functions for KozmoAI Platform CLI."""
...
# Necessary for KozmoAI keywords
MONTHS_VALUE = {
    "JANUARY": 1,
    "FEBRUARY": 2,
    "MARCH": 3,
...
def match_and_return_kozmoai_keyword_date(keyword: str) -> str:  # noqa: PLR0911
    """Return KozmoAI keyword into date.

    Parameters
    ----------
    keyword : str
        String with potential KozmoAI keyword (e.g. 1MONTHAGO,LASTFRIDAY,3YEARSFROMNOW,NEXTTUESDAY)
...
def parse_kozmoai_script(  # noqa: PLR0911,PLR0912
    raw_lines: List[str],
    script_inputs: Optional[List[str]] = None,
) -> Tuple[str, str]:
    """Parse .kozmoai script.

    Parameters
    ----------
    raw_lines : List[str]
        Lines from .kozmoai script
    ...
    str
        Processed string from .kozmoai script that can be run by the KozmoAI Platform CLI
    """
    ...
    # Now we want to replace the ROUTINE_VARS to where applicable throughout the .kozmoai script
    ...
    # recognized by the KozmoAI Platform CLI
    if final_lines:
