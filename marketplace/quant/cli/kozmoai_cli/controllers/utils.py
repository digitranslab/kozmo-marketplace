...
import pandas as pd
import requests
from kozmoai_cli.config.constants import AVAILABLE_FLAIRS, ENV_FILE_SETTINGS
...
def print_goodbye():
    ...
    text = """
[param]Thank you for using the KozmoAI Platform CLI and being part of this journey.[/param]

We hope you'll find the new KozmoAI Platform CLI a valuable tool.

To stay tuned, sign up for our newsletter: [cmds]https://kozmoai.co/newsletter.[/]

Please feel free to check out our other products:

[bold]KozmoAI Terminal Pro[/]: [cmds]https://kozmoai.co/products/pro[/cmds]
[bold]KozmoAI Platform:[/]     [cmds]https://kozmoai.co/products/platform[/cmds]
[bold]KozmoAI Bot[/]:          [cmds]https://kozmoai.co/products/bot[/cmds]
    """
...
def welcome_message():
    ...
    """
    session.console.print(
        f"\nWelcome to KozmoAI Platform CLI v{session.settings.VERSION}"
    )
...
def reset(queue: Optional[List[str]] = None):
    ...
    try:
        ...
        # we clear all kozmoai_cli modules from sys.modules
        for module in list(sys.modules.keys()):
            parts = module.split(".")
            if parts[0] == "kozmoai_cli":
        ...
        if session.is_local():
            from kozmoai_cli.controllers.cli_controller import main
...
def parse_and_split_input(an_input: str, custom_filters: List) -> List[str]:
    ...
    file_flag = r"(\ -f |\ --file )"
    up_to = r".*?"
    known_extensions = r"(\.(xlsx|csv|xls|tsv|json|yaml|ini|kozmoai|ipynb))"
...
def get_user_timezone_or_invalid() -> str:
    ...
    -------
    str
        user timezone based on timezone.kozmoai file or INVALID
    """
...
def compose_export_path(func_name: str, dir_path: str) -> Path:
    ...
    # check if this is done in a main context to avoid saving with kozmoai_cli
    if resolve_path.parts[-2] == "kozmoai_cli":
...
def export_data(
    ...
    figure: Optional["KozmoAIFigure"] = None,
    margin: bool = True,
) -> None:
    ...
    if export_type:
        ...
        for exp_type in export_type.split(","):
            ...
            # In this scenario we use the default filename
                ...
                if ".KozmoAI_kozmoai_cli" in saved_path.name:
                    saved_path = saved_path.with_name(
                        saved_path.name.replace(".KozmoAI_kozmoai_cli", "KozmoAICLI")
                    )
...
def handle_obbject_display(
    ...
    **kwargs,
    ...
    df: pd.DataFrame = pd.DataFrame()
    fig: Optional[KozmoAIFigure] = None
    if chart:
        try:
            if obbject.chart:
                obbject.show(**kwargs)
