...
from kozmoai_cli.config import constants
from kozmoai_cli.config.constants import (
    ASSETS_DIRECTORY,
    ENV_FILE_SETTINGS,
    HOME_DIRECTORY,
    REPOSITORY_DIRECTORY,
)
from kozmoai_cli.config.menu_text import MenuText
from kozmoai_cli.controllers.base_controller import BaseController
from kozmoai_cli.controllers.platform_controller_factory import (
    PlatformControllerFactory,
)
from kozmoai_cli.controllers.script_parser import is_reset, parse_kozmoai_script
from kozmoai_cli.controllers.utils import (
    bootup,
    first_time_user,
...
from kozmoai_cli.session import Session
...
class CLIController(BaseController):
    ...
    def update_runtime_choices(self):
        ...
        if session.prompt_session and session.settings.USE_PROMPT_TOOLKIT:
            ...
            self.ROUTINE_FILES = {
                ...
                for filepath in routines_directory.rglob("*.kozmoai")
            }
    ...
    def print_help(self):
        ...
        mt.add_raw("\n")
        mt.add_info("Record and execute your own .kozmoai routine scripts")
        mt.add_cmd("record", description="start recording current session")
        mt.add_cmd(
            "stop", description="stop session recording and convert to .kozmoai routine"
        )
        mt.add_cmd(
            "exe",
            description="execute .kozmoai routine scripts (use exe --example for an example)",
        )
        mt.add_raw("\n")
    ...
    def call_exe(self, other_args: List[str]):
        ...
        parser.add_argument(
            "--file",
            "-f",
            help="The path or .kozmoai file to run.",
            dest="file",
            ...
            and "my.kozmoai" not in other_args[0],
            type=str,
            nargs="+",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--url", help="URL to run kozmoai script from.", dest="url", type=str
        )
        ...
        if ns_parser:
            if ns_parser.example:
                routine_path = ASSETS_DIRECTORY / "routines" / "routine_example.kozmoai"
                ...
                file_name = f"{username}_{script_name}.kozmoai"
                ...
                # if string is not in this format "default/file.kozmoai" then check for files in ROUTINE_FILES
                full_path = file_path
                hub_routine = file_path.split("/")  # type: ignore
                # Change with: my.kozmoai.co
            ...
            try:
                ...
                err, parsed_script = parse_kozmoai_script(
    ...
    if not jobs_cmds:
        ...
        if first_time_user():
            with contextlib.suppress(EOFError):
                webbrowser.open("https://docs.kozmoai.co/cli")
...
def run_scripts(
    ...
    """Run given .kozmoai scripts.

    Parameters
    ----------
    path : str
        The location of the .kozmoai file
    test_mode : bool
...
def run_routine(file: str, routines_args=Optional[str]):
    """Execute command routine from .kozmoai file."""
    ...
    if user_routine_path.exists():
        ...
        session.console.print(
            f"Routine not found, please put your `.kozmoai` file into : {user_routine_path}."
        )
...
def main(
    ...
    **kwargs,
    ...
    if dev:
        ...
        session.settings.BASE_URL = "https://payments.kozmoai.dev/"
        session.settings.HUB_URL = "https://my.kozmoai.dev"

    if isinstance(path_list, list) and path_list[0].endswith(".kozmoai"):
...
def parse_args_and_run():
    ...
    parser = argparse.ArgumentParser(
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        prog="cli",
        description="The KozmoAI Platform CLI.",
    )
    parser.add_argument(
        "-d",
        "--debug",
