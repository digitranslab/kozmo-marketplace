...
import pandas as pd
from kozmoai_cli.config.completer import NestedCompleter
from kozmoai_cli.config.constants import SCRIPT_TAGS
from kozmoai_cli.controllers.choices import build_controller_choice_map
from kozmoai_cli.controllers.hub_service import upload_routine
...
class BaseController(metaclass=ABCMeta):
    ...
    def call_record(self, other_args) -> None:
        ...
        parser = argparse.ArgumentParser(
            ...
            prog="record",
            description="Start recording session into .kozmoai routine file",
        )
        parser.add_argument(
            "-n",
            "--name",
        ...
        parser.add_argument(
            ...
            default=f"Routine recorded at {datetime.now().strftime('%H:%M')} from the KozmoAI Platform CLI",
            nargs="+",
        )
        parser.add_argument(
            "--tag1",
        ...
        if ns_parser:
            ...
            if session.is_local():
                session.console.print(
                    "[red]Recording session to the KozmoAI Hub is not supported in guest mode.[/red]"
                )
                session.console.print(
                    "\n[yellow]Visit the KozmoAI Hub to register: http://my.kozmoai.co[/yellow]"
                )
                session.console.print(
    ...
    def call_stop(self, other_args) -> None:
        ...
        parser = argparse.ArgumentParser(
            ...
            formatter_class=argparse.ArgumentDefaultsHelpFormatter,
            prog="stop",
            description="Stop recording session into .kozmoai routine file",
        )
        ...
        if "-h" not in other_args and "--help" not in other_args:
            ...
            if not RECORD_SESSION:
                ...
                if RECORD_SESSION_LOCAL_ONLY:
                    # Whitespaces are replaced by underscores and an .kozmoai extension is added
                    title_for_local_storage = (
                        SESSION_RECORDED_NAME.replace(" ", "_") + ".kozmoai"
                    )
                    ...
                    with open(routine_file, "w") as file1:
                        lines = ["# KozmoAI Platform CLI - Routine", "\n"]
