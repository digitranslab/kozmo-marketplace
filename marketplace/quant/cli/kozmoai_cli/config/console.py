...
from kozmoai_cli.config.menu_text import RICH_TAGS
            ...
            if not self._settings.TEST_MODE:
                if self._settings.ENABLE_RICH_PANEL:
                    if self._settings.SHOW_VERSION:
                        ...
                        version = f"[param]KozmoAI Platform CLI v{version}[/param] (https://kozmoai.co)"
                    else:
                        version = (
                            "[param]KozmoAI Platform CLI[/param] (https://kozmoai.co)"
                        )
                    self._console.print(
                        panel.Panel(
                            "\n" + kwargs["text"],
