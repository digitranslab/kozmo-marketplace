...
import pytest
from kozmoai_cli.controllers.script_parser import (
    match_and_return_kozmoai_keyword_date,
    parse_kozmoai_script,
)
...
def test_is_reset(command, expected):
    ...
    from kozmoai_cli.controllers.script_parser import is_reset

    assert is_reset(command) == expected


def test_match_and_return_kozmoai_keyword_date():
    """Test the match_and_return_kozmoai_keyword_date function."""
    keyword = "$LASTFRIDAY"
    result = match_and_return_kozmoai_keyword_date(keyword)
    expected = ""
...
def test_parse_kozmoai_script_basic():
    """Test the parse_kozmoai_script function."""
    raw_lines = ["echo 'Hello World'"]
    error, script = parse_kozmoai_script(raw_lines)
    assert error == ""
    assert script == "/echo 'Hello World'"


def test_parse_kozmoai_script_with_variable():
    """Test the parse_kozmoai_script function."""
...
def test_parse_kozmoai_script_with_foreach_loop():
    """Test the parse_kozmoai_script function."""
...
def test_parse_kozmoai_script_with_error():
    """Test the parse_kozmoai_script function."""
...
def test_parse_kozmoai_script_foreach_errors(line, expected):
    """Test the parse_kozmoai_script function."""
    error, script = parse_kozmoai_script([line])
...
def test_date_keyword_last_friday():
    ...
    assert match_and_return_kozmoai_keyword_date("$LASTFRIDAY") == expected_date
