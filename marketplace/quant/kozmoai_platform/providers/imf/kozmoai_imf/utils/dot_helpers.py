...
def load_country_map() -> dict:
    ...
    try:
        ...
        raise KozmoAIError(f"Failed to load IMF DOT country map: {e}") from e
...
def load_country_to_code_map() -> dict:
    ...
    try:
        ...
        warn(f"Failed to load country to code map. -> {KozmoAIError}")
        return {}
...
def validate_countries(countries: list[str]) -> str:
    ...
    try:
        ...
        raise KozmoAIError(f"Failed to load country to code map: {e}") from e
    if not country_to_code_map:
        raise KozmoAIError("Failed to load country to code map.")
