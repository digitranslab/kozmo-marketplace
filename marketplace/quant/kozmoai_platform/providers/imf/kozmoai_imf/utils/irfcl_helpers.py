...
def load_irfcl_symbols() -> dict:
    ...
    from kozmoai_imf.utils.constants import load_symbols
...
def load_country_map() -> dict:
    ...
    try:
        ...
        raise KozmoAIError(f"Failed to load IMF IRFCL country map: {e}") from e
...
def validate_countries(countries) -> str:
    ...
    if isinstance(countries, str):
        ...
        raise KozmoAIError("Invalid countries list.")
    ...
    if not new_countries:
        raise KozmoAIError("No valid countries found in the supplied list.")
...
def validate_symbols(symbols) -> str:
    ...
    from warnings import warn  # noqa
    from kozmoai_core.app.model.abstract.error import KozmoAIError
    from kozmoai_imf.utils.constants import IRFCL_PRESET
    ...
    if isinstance(symbols, str):
        ...
        raise KozmoAIError("Invalid symbols list.")
...
async def _get_irfcl_data(**kwargs) -> list[dict]:
    ...
    # pylint: disable=import-outside-toplevel
    from kozmoai_core.app.model.abstract.error import KozmoAIError
    from kozmoai_imf.utils import constants
    from kozmoai_imf.utils.helpers import get_data
    ...
    if not indicators and not countries:
        raise KozmoAIError("Country is required when returning the complete dataset.")
    ...
    if not data:
        raise KozmoAIError(f"No data found for '{indicator}' in '{countries}'.")
