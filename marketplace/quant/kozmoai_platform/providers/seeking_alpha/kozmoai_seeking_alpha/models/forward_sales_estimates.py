...
from kozmoai_core.provider.standard_models.forward_sales_estimates import (
    ForwardSalesEstimatesData,
    ForwardSalesEstimatesQueryParams,
)
from kozmoai_core.provider.utils.helpers import amake_request
from kozmoai_seeking_alpha.utils.helpers import HEADERS, get_seekingalpha_id
...
class SAForwardSalesEstimatesQueryParams(ForwardSalesEstimatesQueryParams):
    ...
    def check_symbol(cls, value):
        """Check the symbol."""
        if not value:
            raise KozmoAIError("Symbol is a required field for Seeking Alpha.")
        return value
...
class SAForwardSalesEstimatesFetcher(
    ...
    async def aextract_data(
        ...
        **kwargs: Any,
        ...
        from kozmoai_core.provider.utils.client import ClientSession
        ...
        if not estimates:
            raise KozmoAIError(f"No estimates data was returned for: {query.symbol}")
