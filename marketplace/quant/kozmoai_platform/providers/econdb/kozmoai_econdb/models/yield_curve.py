...
from kozmoai_core.provider.standard_models.yield_curve import (
    YieldCurveData,
...
from kozmoai_econdb.utils.yield_curves import COUNTRIES_DICT, COUNTRIES_LIST
    ...
    async def aextract_data(  # pylint: disable=R0914.R0912,R0915
        ...
        **kwargs: Any,
        ...
        if not results and messages:
            msg_str = "\n".join(messages)
            raise KozmoAIError(msg_str)

        if not results and not messages:
            raise KozmoAIError("Unexpected outcome -> All requests were returned empty.")
