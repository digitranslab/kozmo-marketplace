...
from kozmoai_core.provider.standard_models.immediate_interest_rate import (
...
from kozmoai_core.provider.utils.helpers import check_item
from kozmoai_oecd.utils.constants import CODE_TO_COUNTRY_IR, COUNTRY_TO_CODE_IR
...
class OECDImmediateInterestRateQueryParams(ImmediateInterestRateQueryParams):
    ...
    def validate_country(cls, c):
        ...
        if result:
            return ",".join(result)
        raise KozmoAIError(f"No valid country found. -> {values}")
...
class OECDImmediateInterestRateFetcher(
    ...
    def extract_data(
        ...
        **kwargs: Any,
        ...
        from kozmoai_oecd.utils.helpers import oecd_date_to_python_date  # noqa
        from pandas import read_csv  # noqa
        from kozmoai_core.provider.utils.helpers import make_request  # noqa
