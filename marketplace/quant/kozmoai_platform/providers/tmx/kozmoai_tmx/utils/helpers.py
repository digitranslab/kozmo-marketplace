...
from kozmoai_tmx.utils import gql
...
def get_companies_backend():
    ...
    from kozmoai_core.app.utils import get_user_cache_directory  # noqa
...
async def get_all_etfs(use_cache: bool = True) -> List[Dict]:
    ...
    if not response or response is None:
        raise KozmoAIError("There was a problem with the request. Could not get ETFs.")
...
async def get_all_options_tickers(use_cache: bool = True) -> "DataFrame":
    ...
    from kozmoai_core.provider.utils.helpers import to_snake_case  # noqa
    ...
    if r is None or r == []:
        raise KozmoAIError("Error with the request")  # mypy: ignore
...
async def download_eod_chains(
    ...
    if r is None:
        raise KozmoAIError("Error with the request, no data was returned.")
