...
from kozmoai_core.provider.standard_models.company_filings import (
    CompanyFilingsData,
...
from kozmoai_sec.utils.definitions import FORM_LIST, FORM_TYPES, HEADERS
...
class SecCompanyFilingsQueryParams(CompanyFilingsQueryParams):
    ...
    def validate_form_type(cls, v):
        ...
        if isinstance(v, str):
            ...
            raise KozmoAIError("Unexpected form_type value.")
        new_forms: list = []
        messages: list = []
        for form in forms:
        ...
        if not new_forms:
            raise KozmoAIError(f"No valid forms provided -> {', '.join(messages)}")
...
class SecCompanyFilingsFetcher(
    ...
    async def aextract_data(
        ...
        **kwargs: Any,
        ...
        from kozmoai_sec.utils.helpers import symbol_map
        ...
        if query.symbol and not query.cik:
            ...
            if not query.cik:
                raise KozmoAIError(f"CIK not found for symbol {query.symbol}")
        if query.cik is None:
            raise KozmoAIError("CIK or symbol must be provided.")
