...
async def get_13f_candidates(symbol: Optional[str] = None, cik: Optional[str] = None):
    ...
    if len(filings) == 0:
        raise KozmoAIError(f"No 13F-HR filings found for {symbol if symbol else cik}.")
...
async def complete_submission_callback(response, _):
    ...
    raise KozmoAIError(f"Request failed with status code {response.status}")
...
async def get_complete_submission(url: str):
    ...
    from kozmoai_sec.utils.definitions import HEADERS
