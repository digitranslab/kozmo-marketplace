...
from kozmoai_core.provider.utils.helpers import amake_request, make_request
from kozmoai_sec.utils.definitions import HEADERS, SEC_HEADERS
...
async def get_series_id(
    ...
    if not symbol and not cik:
        raise KozmoAIError("Either symbol or cik must be provided.")
...
async def get_nport_candidates(symbol: str, use_cache: bool = True) -> List[Dict]:
    ...
    try:
        ...
        raise KozmoAIError("Fund not found for, the symbol: " + symbol) from e
    if series_id == "" or series_id is None:
        raise KozmoAIError("Fund not found for, the symbol: " + symbol)
