...
from kozmoai_core.provider.standard_models.company_news import (
    CompanyNewsData,
    CompanyNewsQueryParams,
)
from kozmoai_core.provider.utils.errors import EmptyDataError, UnauthorizedError
from kozmoai_core.provider.utils.helpers import (
    amake_request,
    get_querystring,
)
from kozmoai_intrinio.utils.helpers import get_data
from kozmoai_intrinio.utils.references import IntrinioSecurity
...
class IntrinioCompanyNewsQueryParams(CompanyNewsQueryParams):
    ...
    def _symbol_mandatory(cls, v):
        ...
        if not v:
            raise KozmoAIError("Required field missing -> symbol")
        return v
        ...
        **kwargs: Any,
        ...
        async def callback(response, session):
            ...
            if isinstance(result, dict) and "error" in result:
                ...
                raise KozmoAIError(f"Error in Intrinio request -> {result}")
