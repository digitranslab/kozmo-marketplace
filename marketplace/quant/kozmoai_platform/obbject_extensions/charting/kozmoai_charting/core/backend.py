...
class Backend(PyWry):
    ...
    def __init__(
        ...
        daemon: bool = True,
        max_retries: int = 30,
        proc_name: str = "KozmoAI Platform",
    ):
    ...
    def get_kwargs(self, title: Optional[str] = "") -> dict:
        ...
        return {
            "title": "KozmoAI Platform" + (f" - {title}" if title else ""),
...
def create_backend(charting_settings: Optional["ChartingSettings"] = None):
    ...
    charting_module = importlib.import_module(
        "kozmoai_core.app.model.charts.charting_settings", "charting_settings"
    )
