...
from kozmoai_charting.core.backend import PLOTLYJS_PATH, create_backend, get_backend
from kozmoai_charting.core.config.kozmoai_styles import (
    PLT_TBL_ROW_COLORS,
)

if TYPE_CHECKING:
    from numpy import ndarray
    from kozmoai_core.app.model.charts.charting_settings import ChartingSettings
...
class KozmoAIFigure(go.Figure):
    """Custom Figure class for KozmoAI Terminal.
    ...
    **kwargs
    ...
    Class Methods
    -------------
    create_subplots(rows: `int`, cols: `int`, **kwargs) -> `KozmoAIFigure`
    ...
    to_subplot(subplot: `KozmoAIFigure`, row: `int`, col: `int`, secondary_y: `bool`, ...)
    ...
    def to_subplot(
        ...
        **kwargs,
        ...
        subplot : `plotly.graph_objects.Figure`
            Figure object created with `KozmoAIFigure.create_subplots` / `plotly.subplots.make_subplots`
        row : `int`
            Row number
        col : `int`
            Column number
    ...
    def _add_cmd_source(self, command_location: Optional[str] = "") -> None:
        """Set the watermark for KozmoAI Terminal."""
        if command_location:
