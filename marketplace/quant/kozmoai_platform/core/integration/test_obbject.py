...
@pytest.mark.skipif(
    "kozmoai_charting" not in sys.modules, reason="kozmoai_charting not installed"
)
@pytest.mark.integration
def test_to_chart(obb):
    ...
    assert isinstance(res.chart.fig, KozmoAIFigure)
...
def test_show(obb):
    ...
    assert isinstance(stocks_data.chart.fig, KozmoAIFigure)
