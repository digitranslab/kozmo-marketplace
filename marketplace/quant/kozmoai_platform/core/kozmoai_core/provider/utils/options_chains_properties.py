...
class OptionsChainsProperties(Data):
    ...
    def dataframe(self) -> "DataFrame":
        ...
        if chains_data.empty:
            raise KozmoAIError("Error: No validated data was found.")
    ...
    def total_dex(self) -> Dict:
        ...
        if not self.has_greeks:
            raise KozmoAIError("Greeks are not available.")
    ...
    def filter_data(
        ...
        if stat is not None:
            if stat not in stats:
                raise KozmoAIError(f"Error: stat must be one of {stats}")
        ...
        if column is not None:
            if column not in df.columns:
                raise KozmoAIError(f"Error: column '{column}' not found in data")
    ...
    def _get_stat(
        ...
        if metric in ["DEX", "GEX"]:
            if not self.has_greeks:
                raise KozmoAIError("Greeks were not found within the data.")
    ...
    def _get_nearest_strike(
        ...
        if option_type not in ["call", "put"]:
            raise KozmoAIError("Error: option_type must be either 'call' or 'put'")
    ...
    def strategies(  # noqa: PLR0912
        ...
        if strategies.empty:
            raise KozmoAIError("No strategies found for the given parameters.")
    ...
    def skew(
        ...
        if self.has_iv is False:
            raise KozmoAIError("Error: 'implied_volatility' field not found.")
