...
from kozmoai_core.provider.query_executor import QueryExecutor
from kozmoai_core.provider.registry_map import MapType, RegistryMap
...
class ProviderInterface(metaclass=SingletonMeta):
    ...
    def _create_field(
        ...
        provider_field = (
            f"(provider: {provider_name})" if provider_name != "kozmoai" else ""
        )
        description = (
    ...
    def _extract_params(
        ...
        for provider_name, model_details in providers.items():
            if provider_name == "kozmoai":
                ...
                for name, field in model_details["QueryParams"]["fields"].items():
                    if name not in providers["kozmoai"]["QueryParams"]["fields"]:
            ...
            if provider_name == "kozmoai":
                for name, field in model_details["Data"]["fields"].items():
                    if (
                        ...
                        continue
                    incoming = cls._create_field(name, field, "kozmoai")
                ...
                for name, field in model_details["Data"]["fields"].items():
                    if name not in providers["kozmoai"]["Data"]["fields"]:
                        if (
                            name == "provider"
                            and field.description == "The data provider for the data."
                        ):  # noqa
    ...
    def _generate_model_providers_dc(self, map_: MapType) -> Dict[str, ProviderChoices]:
        ...
        for model_name, providers in map_.items():
            ...
            if "kozmoai" in choices:
                choices.remove("kozmoai")
