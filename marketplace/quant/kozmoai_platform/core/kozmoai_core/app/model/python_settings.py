...
class PythonSettings(BaseModel):
    ...
    http: Optional[dict] = Field(
        ...
        The settings are passed into the `requests.Session` object and the `aiohttp.ClientSession` object by:
            - `kozmoai_core.provider.utils.helpers.make_request` - Sync
            - `kozmoai_core.provider.utils.helpers.amake_request` - Async
            - `kozmoai_core.provider.utils.helpers.amake_requests` - Async (multiple requests)
            - Inserted to use with YFinance & Finviz library implementations.

        Return a session object with the settings applied by:
            - `kozmoai_core.provider.utils.helpers.get_requests_session`
            - `kozmoai_core.provider.utils.helpers.get_async_requests_session`
        """,
    )
    uvicorn: Optional[dict] = Field(
        ...
        + """
            - Running the FastAPI as a Python module script.
              - python -m kozmoai_core.api.rest_api
            - Running the `kozmoai-api` command.
                - kozmoai-api
