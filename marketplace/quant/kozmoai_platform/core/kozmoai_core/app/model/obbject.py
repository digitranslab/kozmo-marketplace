...
from kozmoai_core.app.model.abstract.warning import Warning_
from kozmoai_core.app.model.charts.chart import Chart
from kozmoai_core.provider.abstract.annotated_result import AnnotatedResult
...
if TYPE_CHECKING:
    ...
    from kozmoai_core.app.query import Query  # noqa
...
class OBBject(Tagged, Generic[T]):
    ...
    def to_dataframe(
        ...
        if self.results is None or not self.results:
            raise KozmoAIError("Results not found.")
        ...
        try:
            ...
            if df is None:
                raise KozmoAIError("Unsupported data format.")
            ...
            raise KozmoAIError(f"An unexpected error occurred: {ex}") from ex
    ...
    def show(self, **kwargs: Any) -> None:
        ...
        if not self.chart or not self.chart.fig:
            raise KozmoAIError("Chart not found.")
