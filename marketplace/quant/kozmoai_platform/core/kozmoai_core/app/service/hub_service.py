...
class HubService:
    ...
    def connect(
        ...
        if pat:
            ...
            return self._session
        raise KozmoAIError("Please provide 'email' and 'password' or 'pat'")
    ...
    def _get_session_from_email_password(self, email: str, password: str) -> HubSession:
        ...
        if not email:
            raise KozmoAIError("Email not found.")

        if not password:
            raise KozmoAIError("Password not found.")
    ...
    def _get_session_from_platform_token(self, token: str) -> HubSession:
        ...
        if not token:
            raise KozmoAIError("Platform personal access token not found.")
    ...
    def hub2platform(self, settings: HubUserSettings) -> Tuple[Credentials, Defaults]:
        ...
        if deprecated:
            ...
            warn(
                ...
                "\n\nYou can update them at https://my.kozmoai.co/app/platform/credentials.",
            )
    ...
    def _check_token_expiration(token: str) -> None:
        ...
        try:
            ...
            raise KozmoAIError("Platform personal access token expired.") from e
        except PyJWTError as e:
            raise KozmoAIError("Failed to decode Platform token.") from e
