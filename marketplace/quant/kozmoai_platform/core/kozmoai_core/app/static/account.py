...
from kozmoai_core.app.model.hub.hub_session import HubSession
...
if TYPE_CHECKING:
    from kozmoai_core.app.static.app_factory import BaseApp
...
class Account:  # noqa: D205, D400
    ...
    def __init__(self, base_app: "BaseApp"):
        ...
        self._kozmoai_directory = (
            base_app._command_runner.system_settings.kozmoai_directory
        )
    ...
    def _create_hub_service(
        ...
        if email is None and password is None and pat is None:
            session_file = Path(self._kozmoai_directory, self.SESSION_FILE)
            if not session_file.exists():
                raise KozmoAIError("Session not found.")
    ...
    def login(
        ...
        if remember_me:
            Path(self._kozmoai_directory).mkdir(parents=False, exist_ok=True)
            session_file = Path(self._kozmoai_directory, self.SESSION_FILE)
            with open(session_file, "w") as f:
                if not self._hub_service.session:
                    raise KozmoAIError("Not connected to hub.")
