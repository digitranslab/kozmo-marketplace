...
from kozmoai_core.app.extension_loader import ExtensionLoader, KozmoAIGroups
from kozmoai_core.app.model.example import Example
from kozmoai_core.app.model.field import KozmoAIField
...
from kozmoai_core.app.static.utils.console import Console
from kozmoai_core.app.static.utils.linters import Linters
from kozmoai_core.app.version import CORE_VERSION, VERSION
...
if TYPE_CHECKING:
    ...
    from kozmoai_core.provider.abstract.data import Data  # noqa

try:
    from kozmoai_charting import Charting  # type: ignore
...
class PackageBuilder:
    ...
    def _save_reference_file(self, ext_map: Optional[Dict[str, List[str]]] = None):
        ...
        code = dumps(
            obj={
                "kozmoai": VERSION.replace("dev", ""),
                "info": {
                    "title": "KozmoAI Platform (Python)",
            ...
            Example:
                {
                    ...
                    "kozmoai_obbject_extension": [
                        "kozmoai_charting@1.0.0",
                        ...
                    ]
                }
...
class ImportDefinition:
    ...
    def build(cls, path: str) -> str:
        ...
        code = "from kozmoai_core.app.static.container import Container"
        code += "\nfrom kozmoai_core.app.model.obbject import OBBject"
        ...
        code += "\nfrom kozmoai_core.app.static.utils.decorators import exception_handler, validate\n"
        code += "\nfrom kozmoai_core.app.static.utils.filters import filter_inputs\n"
    ...
    def build(path: str, ext_map: Optional[Dict[str, List[str]]] = None) -> str:
        ...
        if not path:
            if ext_map:
                ...
                doc += "Extensions:\n"
                doc += "\n".join(
                    [f"    - {ext}" for ext in ext_map.get("kozmoai_core_extension", [])]
                )
                doc += "\n\n"
                doc += "\n".join(
                    [
                        f"    - {ext}"
                        for ext in ext_map.get("kozmoai_provider_extension", [])
                    ]
                )
            doc += '    """\n'
            doc += "    # fmt: on\n"
...
class MethodDefinition:
    ...
    def build_func_params(formatted_params: OrderedDict[str, Parameter]) -> str:
        ...
        func_params = func_params.replace(
            "kozmoai_core.provider.abstract.data.Data", "Data"
        )
...
class DocstringGenerator:
    ...
    def get_field_type(
        ...
        try:
            ...
            if "kozmoai_" in str(_type):
                _type = (
                    str(_type).split(".", maxsplit=1)[0].split("kozmoai_")[0]
    ...
    def build_examples(
        ...
        if examples:
            ...
            doc = f"\n{indent}Examples\n"
            doc += f"{indent}--------\n"
            doc += f"{indent}{prompt}from kozmoai import obb\n"
...
class ReferenceGenerator:
    ...
    def _get_provider_field_params(
        ...
        provider : str
            Provider name. Defaults to "kozmoai".
        ...
        for path, route in route_map.items():
            ...
            if route_method == {"GET"} and model_map:
                ...
                for provider in model_map:
                    if provider == "kozmoai":
                        # kozmoai provider is always present hence its the standard field
                        reference[path]["parameters"]["standard"] = (
                            cls._get_provider_field_params(
                                standard_model, "QueryParams"
                            )
                        )
                        # Add `provider` parameter fields to the kozmoai provider
