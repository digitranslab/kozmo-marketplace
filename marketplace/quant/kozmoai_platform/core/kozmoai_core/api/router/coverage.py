...
from kozmoai_core.api.dependency.coverage import get_command_map, get_provider_interface
...
async def get_commands_model_map(
    ...
    for command in command_map.commands_model:
        ...
        for provider in providers:
            ...
            if kozmoai_info := new_command.get("kozmoai", {}):
                for key in list(new_command):
                    if key == "response_schema_name":
                        continue

                    if obb_params := kozmoai_info.get("QueryParams", {}).get(
                        "fields", {}
                    ):
