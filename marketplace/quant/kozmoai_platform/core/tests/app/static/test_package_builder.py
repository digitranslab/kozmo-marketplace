...
from kozmoai_core.app.static.package_builder import (
    ClassDefinition,
    DocstringGenerator,
    ImportDefinition,
    MethodDefinition,
...
def tmp_kozmoai_dir(tmp_path_factory):
    """Return a temporary kozmoai directory."""
    return tmp_path_factory.mktemp("kozmoai")


@pytest.fixture(scope="module")
def package_builder(tmp_kozmoai_dir):
    """Return package builder."""
    return PackageBuilder(tmp_kozmoai_dir)
...
def test_build(class_definition):
    """Test build."""
    code = class_definition.build("kozmoai_core.app.static.container.Container")
    assert code
...
def test_build_class_loader_method(method_definition):
    ...
    code = method_definition.build_class_loader_method(
        "kozmoai_core.app.static.container.Container"
    )
    assert code
...
@patch("kozmoai_core.app.static.package_builder.MethodDefinition")
...
def test_build_command_method_body(method_definition):
    ...
    with patch(
        "kozmoai_core.app.static.package_builder.MethodDefinition.is_data_processing_function",
        return_value=False,
    ), patch(
        "kozmoai_core.app.static.package_builder.MethodDefinition.is_deprecated_function",
        return_value=False,
    ):
        output = method_definition.build_command_method_body(
            path="kozmoai_core.app.static.container.Container", func=some_func
        )
...
def test_build_command_method(method_definition):
    ...
    with patch(
        ...
        output = method_definition.build_command_method(
            path="kozmoai_core.app.static.container.Container",
            func=some_func,
            model_name=None,
        )
...
def test_import_definition_build(import_definition):
    ...
    code = import_definition.build(path="kozmoai_core.app.static.container.Container")
    assert code
...
def test__read(package_builder, tmp_kozmoai_dir):
    """Test read."""

    PATH = "kozmoai_core.app.static.package_builder."
    open_mock = mock_open()
    with patch(PATH + "open", open_mock), patch(PATH + "load") as mock_load:
        package_builder._read(Path(tmp_kozmoai_dir / "assets" / "reference.json"))
        open_mock.assert_called_once_with(
            Path(tmp_kozmoai_dir / "assets" / "reference.json")
        )
...
@pytest.mark.parametrize(
    ...
    [
        (
            {
                "kozmoai_core_extension": [
                    "ext_1@0.0.0",
                    "ext_2@0.0.0",
                ],
                "kozmoai_provider_extension": [
                    "prov_1@0.0.0",
                    "prov_2@1.1.1",
                ],
            },
            EntryPoints(
                (
                    EntryPoint(
                        name="ext_2", value="...", group="kozmoai_core_extension"
                    ),
                    EntryPoint(
                        name="prov_2", value="...", group="kozmoai_provider_extension"
                    ),
                )
            ),
            "0.0.0",
        ...
        (
            {
                "kozmoai_core_extension": ["ext_1@9.9.9"],
                "kozmoai_provider_extension": ["prov_2@0.0.0"],
            },
            EntryPoints(
                (
                    EntryPoint(
                    ...
                    EntryPoint(
                        name="prov_1", value="...", group="kozmoai_provider_extension"
                    ),
                )
            ),
            "5.5.5",
...
def test_auto_build(package_builder, add, remove, kozmoai_auto_build):
    """Test auto build."""

    with patch.object(PackageBuilder, "_diff") as mock_assets_diff, patch.object(
        PackageBuilder, "build"
    ) as mock_build, patch.object(Env, "AUTO_BUILD", kozmoai_auto_build):
        mock_assets_diff.return_value = add, remove

        package_builder.auto_build()

    if kozmoai_auto_build:
        if add or remove:
