...
def logging_service():
    ...
    with patch(
        "kozmoai_core.app.logs.logging_service.LoggingSettings",
        MockLoggingSettings,
    ), patch(
        "kozmoai_core.app.logs.logging_service.LoggingService._setup_handlers",
        mock_setup_handlers,
    ), patch(
        "kozmoai_core.app.logs.logging_service.LoggingService._log_startup",
        mock_log_startup,
    ):
...
def test_log_startup(logging_service):
    ...
    logging_service._log_startup(
        route="test_route", custom_headers={"X-KozmoAI-Test": "test"}
    )
    ...
    expected_log_data = {
        ...
        "custom_headers": {"X-KozmoAI-Test": "test"},
    }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            (
                KozmoAIError,
                KozmoAIError("mock_error"),
                ...,
            ),  # ... is of TracebackType, but unnecessary for the test
            {"X-KozmoAI-Test": "test"},
            'ERROR: {"route": "mock_route", "input": {}, "error": "mock_error", "provider": "not_passed_to_kwargs", "custom_headers": {"X-KozmoAI-Test": "test"}}',  # noqa: E501
        ),
        (
            "mock_settings",
            MockSystemSettings(),
            ...
            {},
            (None, None, None),
            {"X-KozmoAI-Test1": "test1", "X-KozmoAI-Test2": "test2"},
            "STARTUP",
        ),
    ],
)
