...
from kozmoai_core.app.service.hub_service import (
    Credentials,
    HubService,
    HubSession,
    HubUserSettings,
...
def mocker():
    """Fixture for mocker."""
    with patch("kozmoai_core.app.service.hub_service.HubService") as mock:
        yield mock
...
def test_v3tov4_map():
    ...
    providers = sorted(
        [
            p.stem
            for p in Path("kozmoai_platform", "providers").glob("*")
...
def test_connect_without_credentials():
    ...
    hub_service = HubService()
    with pytest.raises(
        KozmoAIError, match="Please provide 'email' and 'password' or 'pat'"
    ):
        hub_service.connect()
...
def test__check_token_expiration(offset, message):
    ...
    if message:
        with pytest.raises(KozmoAIError, match=message):
