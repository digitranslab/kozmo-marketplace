...
import pytest
from kozmoai_core.app.extension_loader import EntryPoint, ExtensionLoader, KozmoAIGroups
...
def test_kozmoai_groups():
    """Test the KozmoAIGroups enum."""
    assert len(KozmoAIGroups) == 3
    assert KozmoAIGroups.core.value == "kozmoai_core_extension"
    assert KozmoAIGroups.provider.value == "kozmoai_provider_extension"
    assert KozmoAIGroups.obbject.value == "kozmoai_obbject_extension"
...
def test_obbject_entry_points():
    ...
    for ep in el.obbject_entry_points:
        assert ep.group == KozmoAIGroups.obbject.value
...
def test_core_entry_points():
    ...
    for ep in el.core_entry_points:
        assert ep.group == KozmoAIGroups.core.value
...
def test_provider_entry_points():
    ...
    for ep in el.provider_entry_points:
        assert ep.group == KozmoAIGroups.provider.value


def test_sorted_entry_points():
    """Test the _sorted_entry_points method."""
    # pylint: disable=protected-access
    core_entry_points = ExtensionLoader._sorted_entry_points(KozmoAIGroups.core.value)
...
def test_get_entry_point():
    ...
    if result:
        assert result.group == KozmoAIGroups.provider.value
        assert result.name == "fmp"

    # pylint: disable=protected-access
    result = el._get_entry_point(el.core_entry_points, "equity")
    if result:
        assert result.group == KozmoAIGroups.core.value
...
@patch("kozmoai_core.app.extension_loader.ExtensionLoader._get_entry_point")
def test_get_obbject_entry_point(mock_get_entry_point):
    ...
    mock_get_entry_point.return_value = EntryPoint(
        name="mock_extension", group=KozmoAIGroups.obbject.value, value="mock"
    )

    el = ExtensionLoader()
    result = el.get_obbject_entry_point("mock_extension")
    if result:
        assert result.group == KozmoAIGroups.obbject.value
