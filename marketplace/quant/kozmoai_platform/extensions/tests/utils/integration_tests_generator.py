...
def obb(pytestconfig):  # pylint: disable=inconsistent-return-statements
    ...
    if pytestconfig.getoption("markexpr") != "not integration":
        import kozmoai  # pylint: disable=import-outside-toplevel
...
def get_test_params(
    ...
    standard_params = get_model_params(
        param_fields=provider_interface_map[model_name]["kozmoai"]["QueryParams"][
            "fields"
        ]
    )
...
def write_charting_extension_integration_tests():
    ...
    import kozmoai_charting  # pylint: disable=import-outside-toplevel

    functions = Charting.functions()

    charting_ext_path = Path(kozmoai_charting.__file__).parent.parent
    test_file = charting_ext_path / "integration" / "test_charting_python.py"

    extra = """assert result.chart.content
    assert isinstance(result.chart.fig, KozmoAIFigure)"""
