...
def get_packages_info() -> Dict[str, str]:
    ...
    package_paths = glob.glob("kozmoai_platform/kozmoai/package/*.py")
...
def execute_docstring_examples(module_name: str, path: str) -> List[str]:
    ...
    errors = []
    module = importlib.import_module(f"kozmoai.package.{module_name}")
...
def list_kozmoai_extensions() -> Tuple[Set[str], Set[str], Set[str]]:
    """List installed kozmoai extensions and providers.
    ...
    if hasattr(entry_points_dict, "select"):  # Python 3.12+
        core_entry_points = entry_points_dict.select(group="kozmoai_core_extension")
        provider_entry_points = entry_points_dict.select(
            group="kozmoai_provider_extension"
        )
        obbject_entry_points = entry_points_dict.select(
            group="kozmoai_obbject_extension"
        )
    else:
        core_entry_points = entry_points_dict.get("kozmoai_core_extension", [])
        provider_entry_points = entry_points_dict.get("kozmoai_provider_extension", [])
        obbject_entry_points = entry_points_dict.get("kozmoai_obbject_extension", [])
...
def find_decorator(file_path: str, function_name: str) -> str:
    ...
    file_path = os.path.join(
        this_dir.split("kozmoai_platform/")[0], "kozmoai_platform", file_path
    )
...
def get_required_fields(model: str) -> List[str]:
    ...
    fields = pi.map[model]["kozmoai"]["QueryParams"]["fields"]
