...
def test_default_package_files():
    ...
    with open(ROOT_DIR / "pyproject.toml") as f:
        data = load(f)
    dependencies = data["tool"]["poetry"]["dependencies"]
    package_files = glob.glob("kozmoai_platform/kozmoai/package/*.py")
    ...
    for file_path in package_files:
        ...
        if "_" in package_name:
            ...
            if "kozmoai-" + base_package not in default_packages:
                invalid_packages.append(package_name)
        elif "kozmoai-" + package_name not in default_packages:
            invalid_packages.append(package_name)
