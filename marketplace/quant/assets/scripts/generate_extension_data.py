...
KOZMOAI_PLATFORM_PATH = Path(THIS_DIR, "..", "..", "kozmoai_platform")
PROVIDERS_PATH = KOZMOAI_PLATFORM_PATH / "providers"
EXTENSIONS_PATH = KOZMOAI_PLATFORM_PATH / "extensions"
OBBJECT_EXTENSIONS_PATH = KOZMOAI_PLATFORM_PATH / "obbject_extensions"

KOZMOAI_PLATFORM_TOML = PyProjectTOML(KOZMOAI_PLATFORM_PATH / "pyproject.toml")
...
def create_item(package_name: str, obj: object, obj_attrs: List[str]) -> Dict[str, Any]:
    ...
    pkg_spec = KOZMOAI_PLATFORM_TOML.data["tool"]["poetry"]["dependencies"].get(
        package_name
    )
...
def generate_provider_extensions() -> None:
    ...
    packages = get_packages(PROVIDERS_PATH, "kozmoai_provider_extension")
...
def generate_router_extensions() -> None:
    ...
    packages = get_packages(EXTENSIONS_PATH, "kozmoai_core_extension")
    data: List[Dict[str, Any]] = []
...
def generate_obbject_extensions() -> None:
    ...
    packages = get_packages(OBBJECT_EXTENSIONS_PATH, "kozmoai_obbject_extension")
    data: List[Dict[str, Any]] = []
    obj_attrs = ["description"]
    for pkg_name, details in sorted(packages.items()):
        plugin = details.get("plugin", "")
